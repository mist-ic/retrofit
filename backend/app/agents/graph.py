...
def build_graph() -> StateGraph:
    ...
    # Register agent nodes — each receives (state, writer) in LangGraph 1.x
    ...
    # Entry point: START → ad_analyzer
    graph.add_edge(START, "ad_analyzer")

    # Linear pipeline: ad_analyzer → page_scraper → ... → code_modifier → qa_verifier
    ...
    # Conditional retry: qa_verifier → code_modifier (retry) | END (done)
