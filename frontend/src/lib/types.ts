 ...
 * Keep in sync when Pydantic models change.
 */
...
// ── CROFindings (cro_findings.py) ───────────────────────────────────────────

export type CROCriterionId =
  | "message_match"
  | "hero_clarity"
  | "cta_visibility"
  | "social_proof"
...
export interface ReplaceAttributeOp {
  op: "replaceAttribute";
  selector: string;
  attribute: string;
  new_value: string;
...
export interface ReplaceStyleOp {
  op: "replaceStyle";
  selector: string;
  css_text: string;
}
...
export interface LogEntry {
  ...
  stage: string;
  message: string;
  type: "start" | "progress" | "complete" | "error" | "node" | "system";
}
